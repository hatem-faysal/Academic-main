...
class TeacherController extends Controller
{
    ...
    public function __construct(User $admin){
        $this->Model=$admin;
        $this->messageStore='Success Add Teacher';
        $this->messageUpdate='Update Teacher';
        $this->messageDelete='Success  Delete Teacher';
        $this->route='admin.teacher.index';
    ...
    public function create():View
    {
        ...
        return view('admin.teacher.crud',compact('type','action','method','roles'));
    }
    public function store(StoreAdminsRequest $request):RedirectResponse
    {
        app($this->StoreAction)->handle($request->validated());
        return redirect()->route('admin.teacher.index')->with('add',$this->messageStore);
    }
    public function edit($id):View
    {
        $type = 'Create';
        $action = route('admin.teacher.update', $id);
        $method = 'PUT';
        $admin = $this->Model::find($id);
        $roles = Role::pluck('name','name')->all();
        $AdminRole =  $admin->roles->pluck('name','name')->all();
        return view('admin.teacher.crud',compact('type','action','method','roles','admin','AdminRole'));
    }
